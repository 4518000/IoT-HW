...
7-02 <br/> g 객체는 Grade 생성자 함수로 생성된 객체 <br/> 인덱스 요소 접근 가능 및 배열의 prototype 메서드와 속성 상속 받음

7-03 <br/> delete g.length를 통해 length의 속성을 삭제하고 push 메서드를 호출하면 추가된 요소가 g의 인덱스 0에 저장되고 새로운 length 속성이 1로 설정됨

7-04 <br/> g와 Grade.prototype 사이에 프로토타입 체인 형성 <br/> g.push(70)으로 값이 할당되어있지 않은 인덱스 3을 지나 4가 생성되어 값을 할당
...
7-09 <br/> Bridge 클래스를 통해 슈퍼 클래스의 프로토타입을 상속 받고 이를 서브 클래스의 프로토타입으로 설정

7-10 <br/> Object.freeze로 Square.prototype의 속성이 수정되지 않도록 함
